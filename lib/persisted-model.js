...
module.exports = function(registry) {
  ...
  PersistedModel.setupRemoting = function() {
    ...
    setRemoting(PersistedModel, 'subscribe', {
      ...
      accessType: 'READ',
      http: {verb: 'post', path: '/subscription'},
      accepts: {
        arg: 'options',
        type: 'object'
      },
  ...
  PersistedModel.subscribe = function(options, cb) {
    ...
    changes._read = function() {
      // TODO(ritch) should this not be a readable stream?
      // bad sign there is nothing in the _read method...
      // we are essentially forcing data into the read queue
      // instead of obeying `rs.push(d) ? keepReading() : wait()`
      debug('changes._read');
    };
    ...
    function afterSave(ctx, next) {
      ...
      debug('new change: %j', change);
      changes.push(change);
      next();
    }

    function afterDelete(ctx, next) {
      debug('new change: %j', ctx);
      buffer.push({
        type: 'delete',
        where: ctx.where,
